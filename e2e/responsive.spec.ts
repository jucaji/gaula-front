...
async function mockTodo(page: Page) {
  await page.route('**/api/**', async (route) => {
    ...
    // SPEC-0810: el explorador con nombres largos de verdad, que es donde se rompe una barra.
    if (/observatory\/explore\/catalog/.test(url)) return json([{ code: 'OFFICIAL', label: 'Cifras oficiales de Mindefensa', measure: { code: 'VICTIMS', label: 'Víctimas', singular: 'víctima', plural: 'víctimas' }, dimensions: [{ code: 'year', label: 'Año', kind: 'YEAR', child: null }, { code: 'department', label: 'Departamento', kind: 'CATEGORY', child: 'municipality' }, { code: 'municipality', label: 'Municipio', kind: 'CATEGORY', child: null }] }])
    if (/observatory\/explore$/.test(url)) return json({ source: 'OFFICIAL', dimension: { code: 'department', label: 'Departamento', kind: 'CATEGORY', child: 'municipality' }, measure: { code: 'VICTIMS', label: 'Víctimas', singular: 'víctima', plural: 'víctimas' }, rows: [{ key: '54', label: 'Norte de Santander', value: 1873450, partial: false }, { key: '88', label: 'Archipiélago de San Andrés, Providencia y Santa Catalina', value: 12, partial: false }], others: { key: '__OTHERS__', label: 'Otros (31)', value: 99, partial: false }, total: 1873561, firstDate: '2003-01-01', cutoffDate: '2026-07-30', colorOrder: ['54', '88'], description: 'Víctimas por departamento, cifras oficiales de mindefensa; delito: Secuestro; corte 2026-07-30.' })
    if (/observatory\/visualizations/.test(url)) return json([])
    if (/observatory\/profiles/.test(url)) return json([{ id: 'p1', code: 'FISCALIA_EXTORSION_SECUESTRO', version: 1, provisional: true, displayName: 'Fiscalía — Extorsión y Secuestro (plantilla observada 19/08/2026)', sheets: ['SECUESTRO', 'EXTORSION'], forms: [], validFrom: '2026-01-01' }])
...
const RUTAS = [
  ...
  '/observatorio/hechos', '/observatorio/cargue', '/observatorio/cifras-oficiales', '/observatorio/explorar', '/tableros', '/tableros/extorsion',
