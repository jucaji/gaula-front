...
function BulletinPage() {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      setDownloadError(
        err instanceof Error ? err.message : "No se pudo descargar el boletín.",
      );
    } finally {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/*
        El boletín ES una lámina: tiene un ancho propio y no crece con la
        pantalla. En un monitor de 1920 quedaba pegado a la izquierda con dos
        tercios vacíos a la derecha (reportado por el cliente). Se centra la
        lámina COMPLETA —encabezado y botón incluidos— para que el botón de
        descarga siga alineado con el borde derecho del documento y no se quede
        colgado al otro extremo de la pantalla.
      */}
      <div className="mx-auto w-full max-w-5xl">
        ...
        {isLoading && (
          <p className="mt-6 text-sm text-text-secondary">
            Armando el boletín del corte vigente…
          </p>
        )}

        {isError && (
          <div
            className="mt-6 rounded-sm border border-border-strong bg-surface-raised p-4"
            role="alert"
          >
            ...
            <p className="mt-1 max-w-xl text-sm text-text-secondary">
              ...
              Sin un corte vigente el boletín no se genera: un PDF de ceros con
              la fecha de hoy parecería un dato.
            </p>
          </div>
        )}
          ...
          <article className="mt-4">
            ...
            <section className="rounded-sm border border-border-strong bg-surface-raised p-4">
              <p className="text-sm font-semibold text-text-primary">
                Corte al{" "}
                {CUTOFF_DATE.format(
                  new Date(`${bulletin.cutoffDate}T00:00:00`),
                )}
              </p>
              <dl className="mt-2 grid gap-x-6 gap-y-1 text-2xs text-text-secondary sm:grid-cols-2">
                ...
                <div className="flex gap-2">
                  ...
                  <dd className="text-text-primary">
                    {bulletin.loadedByName ?? "—"}
                  </dd>
                </div>
                <div className="flex gap-2">
                  <dt className="text-text-muted">Generado:</dt>
                  <dd className="text-text-primary">
                    {formatDateTime(bulletin.generatedAt)}
                  </dd>
                </div>
              </dl>
            </section>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <div className="rounded-sm border border-border-strong bg-surface p-3">
                ...
                {bulletin.previousComparison && (
                  <p className="text-2xs text-text-secondary">
                    {bulletin.previousComparison.previousExtortionTotal.toLocaleString(
                      "es-CO",
                    )}{" "}
                    en el corte al{" "}
              ...
              <div className="rounded-sm border border-border-strong bg-surface p-3">
                ...
                {bulletin.previousComparison && (
                  <p className="text-2xs text-text-secondary">
                    {bulletin.previousComparison.previousKidnappingTotal.toLocaleString(
                      "es-CO",
                    )}{" "}
                    en el corte al{" "}
                    {bulletin.previousComparison.previousCutoffDate}
            ...
            {!bulletin.previousComparison && (
              <p className="mt-2 text-2xs text-text-muted">
                No hay un corte anterior comparable, así que el boletín no
                muestra variación. Un porcentaje contra un corte vacío sería una
                cifra inventada.
              </p>
            )}
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2">
              <BreakdownList
                title="Principales grupos autores"
                rows={bulletin.topAuthorGroups}
              />
              <BreakdownList
                title="Departamentos con más hechos"
                rows={bulletin.topDepartments}
              />
              <BreakdownList
                title="Modalidad de extorsión"
                rows={bulletin.byModality}
                label={(key) =>
                  MODALITY_LABEL[key as keyof typeof MODALITY_LABEL] ?? key
                }
              />
              <BreakdownList
                ...
                label={(key) =>
                  VICTIM_STATUS_LABEL[
                    key as keyof typeof VICTIM_STATUS_LABEL
                  ] ?? key
                }
              />
            </div>
